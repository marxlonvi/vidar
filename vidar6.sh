...
monitor_services() {
    ...
    while true; do
        ...
        # 3. Hitung lebar tabel dinamis sesuai lebar terminal (biar tidak wrap di HP)
        TERM_COLS=$(tput cols 2>/dev/null || echo 50)
        [ -z "$TERM_COLS" ] && TERM_COLS=50
        STATUS_W=11
        # sisakan ruang untuk: "| " + nama + " | " + status + " |" = 2+N+3+STATUS_W+2
        PKG_W=$(( TERM_COLS - STATUS_W - 7 ))
        [ "$PKG_W" -lt 10 ] && PKG_W=10

        BORDER=$(printf '+%s+%s+' "$(printf -- '-%.0s' $(seq 1 $((PKG_W+2))))" "$(printf -- '-%.0s' $(seq 1 $((STATUS_W+2))))")

        # 4. Gambar Header Tabel
        echo -e "${CYAN}${BORDER}${NC}"
        printf "${CYAN}| %-${PKG_W}s | %-${STATUS_W}s |${NC}\n" "PACKAGE" "STATUS"
        echo -e "${CYAN}${BORDER}${NC}"
        
        # System & Memory Info
        SYS_STATUS="[${AKTIF_COUNT}/${TOTAL_PKG}]"
        printf "| ${CYAN}%-${PKG_W}s${NC} | ${YELLOW}%-${STATUS_W}s${NC} |\n" "System" "$SYS_STATUS"
        printf "| ${CYAN}%-${PKG_W}s${NC} | %-${STATUS_W}s |\n" "Memory" "$MEM_INFO"
        
        echo -e "${CYAN}${BORDER}${NC}"

        # 5. Gambar Daftar Package (nama dipotong kalau kepanjangan agar tidak wrap)
        for pkg in "${ACTIVE_PACKAGES[@]}"; do
            DISP_PKG="$pkg"
            if [ "${#DISP_PKG}" -gt "$PKG_W" ]; then
                DISP_PKG="${DISP_PKG:0:$((PKG_W-1))}~"
            fi
            if [ "${PKG_STATUS[$pkg]}" == "online" ]; then
                printf "| %-${PKG_W}s | ${GREEN}%-${STATUS_W}s${NC} |\n" "$DISP_PKG" "Online"
            else
                printf "| %-${PKG_W}s | %-${STATUS_W}s |\n" "$DISP_PKG" "Offline"
            fi
        done
